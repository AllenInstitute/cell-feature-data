...
const { firestore } = require("../../firebase/setup-firebase");

const TEMP_LOCAL_FILE_INFO_JSON =
  require("../constants").TEMP_LOCAL_FILE_INFO_JSON;

const uploadFileInfo = async (firebaseHandler, readFolder, skipUpload) => {
  console.log("uploading file info...");
  const json = await readAndParseFile(
    `${readFolder}/${TEMP_LOCAL_FILE_INFO_JSON}`
  );
  ...
  const writeBatch = async () => {
    ...
    if (batchOfData.length) {
      ...
      for (let index = 0; index < batchOfData.length; index++) {
        ...
        const docRef = firebaseHandler.cellRef
          .collection(firebaseHandler.cellFileInfoEndpoint)
          .doc(cellData.CellId.toString());
  ...
  return {
    fileInfoPath: firebaseHandler.cellRef.collection(
      firebaseHandler.cellFileInfoEndpoint
    ).path,
  };
};
